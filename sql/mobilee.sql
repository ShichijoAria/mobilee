...
--
-- Host: localhost
-- Generation Time: 2018-01-27 14:55:13
...
-- 存储过程
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_admin_login` (IN `_temp` VARCHAR(18), IN `_password` VARCHAR(32), IN `_type` INT)  BEGIN
    DECLARE __id BIGINT;
    DECLARE __name VARCHAR(18);
    DECLARE __password VARCHAR(32);
    DECLARE __gender TINYINT;
    DECLARE __created TIMESTAMP;
    DECLARE __phone VARCHAR(13);
    DECLARE __email VARCHAR(18);
    DECLARE __error INTEGER DEFAULT 0;
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET __error=1;

    START TRANSACTION;

    IF _type=0 THEN
      SELECT id,name,password,gender,created,phone,email
      INTO __id,__name,__password,__gender,__created,__phone,__email
      FROM t_admin WHERE name=`_temp` AND password=_password;
    ELSEIF _type=1 THEN
      SELECT id,name,password,gender,created,phone,email
       INTO __id,__name,__password,__gender,__created,__phone,__email
      FROM t_admin WHERE phone=`_temp` AND password=_password;
    ELSE
      SELECT id,name,password,gender,created,phone,email
       INTO __id,__name,__password,__gender,__created,__phone,__email
      FROM t_admin WHERE email=`_temp` AND password=_password;
    END IF;

    IF found_rows()>0 THEN
      INSERT INTO t_admin_login_log(admin)VALUES (__id);
      SELECT __id,__name,__password,__gender,__created,__phone,__email;
    END IF;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_user_login` (IN `_temp` VARCHAR(18), IN `_password` VARCHAR(32), IN `_type` INT)  BEGIN
    DECLARE __id BIGINT;
    DECLARE __name VARCHAR(18);
    DECLARE __password VARCHAR(32);
    DECLARE __gender TINYINT;
    DECLARE __created TIMESTAMP;
    DECLARE __birthday TIMESTAMP;
    DECLARE __phone VARCHAR(13);
    DECLARE __email VARCHAR(18);
    DECLARE __error INTEGER DEFAULT 0;
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET __error=1;
    ...
    IF _type=0 THEN
      SELECT id,name,password,gender,created,birthday,phone,email
      INTO __id,__name,__password,__gender,__created,__birthday,__phone,__email
      FROM t_user WHERE name=`_temp` AND password=_password;
    ELSEIF _type=1 THEN
      SELECT id,name,password,gender,created,birthday,phone,email
      INTO __id,__name,__password,__gender,__created,__birthday,__phone,__email
      FROM t_user WHERE phone=`_temp` AND password=_password;
    ELSE
      SELECT id,name,password,gender,created,birthday,phone,email
      INTO __id,__name,__password,__gender,__created,__birthday,__phone,__email
      FROM t_user WHERE email=`_temp` AND password=_password;
    END IF;

    IF found_rows()>0 THEN
      INSERT INTO t_login_log(user)VALUES (__id);
      SELECT __id,__name,__password,__gender,__created,__birthday,__phone,__email;
    END IF;
...
INSERT INTO `t_admin` (`id`, `name`, `password`, `phone`, `email`, `gender`, `created`) VALUES
(1, '龙傲天', 'd941f8f4487ffd447fa431d80b32a472', NULL, NULL, NULL, '2018-01-22 08:41:30'),
(2, '叶良辰d', 'ed83fb04cfa523f35ad69aaea51daad9', NULL, NULL, NULL, '2018-01-22 08:41:30'),
(3, '赵日天', '22196ee6f4461dc272fef700dca9cc98', NULL, NULL, NULL, '2018-01-22 08:41:30');
...
INSERT INTO `t_admin_login_log` (`id`, `admin`, `created`) VALUES
(1, 1, '2018-01-22 11:27:06'),
(2, 1, '2018-01-22 11:28:20');
...
(34, 4, '2018-01-22 08:22:36'),
(35, 4, '2018-01-22 08:23:31'),
(36, 4, '2018-01-22 08:24:05'),
(37, 4, '2018-01-22 08:28:21'),
(38, 15, '2018-01-22 13:39:19'),
(39, 15, '2018-01-23 04:07:39'),
(40, 15, '2018-01-23 04:08:54'),
(41, 15, '2018-01-23 04:40:28'),
(42, 15, '2018-01-23 05:27:40'),
(43, 15, '2018-01-23 05:38:55'),
(44, 15, '2018-01-23 05:42:51'),
(45, 15, '2018-01-23 05:44:27'),
(46, 15, '2018-01-23 05:54:12'),
(47, 15, '2018-01-23 05:57:38'),
(48, 15, '2018-01-25 06:07:50'),
(49, 15, '2018-01-25 06:23:55'),
(50, 15, '2018-01-25 06:25:26'),
(51, 15, '2018-01-25 06:27:03');
...
(1, '李哥', 'f73b7d7d59e2efc769b5440e55047608', 2, '2017-11-01 09:35:00', '2017-10-30 16:00:00', NULL, NULL, 10),
(2, '夜吹df', '4dd0553053ccc06c3781b9a3935609d4', 1, '2017-11-01 09:35:00', '2017-10-30 16:00:00', NULL, NULL, 9),
(3, '阿卡', '6f2045258e2071210585c6557cffce04', 1, '2017-11-01 09:35:00', '2017-10-30 16:00:00', NULL, NULL, 5),
(4, '卡fd', '58884d2623f1b42d75ee431051d5007b', 1, '2017-11-01 09:35:21', '2017-10-31 10:18:00', NULL, NULL, 4),
(5, '阿卡林112', '56f62efc784f0f1aa8fe0ee83ecf2cab', 1, '2017-11-01 09:35:21', '2017-10-31 10:18:00', NULL, NULL, 4),
(6, '小智df', 'af5c6f05d4309ea70dff3317ed816559', 1, '2017-11-01 09:35:00', '2017-10-30 16:00:00', NULL, NULL, 6),
(7, '阿卡林1', 'd2d02019521158602ce9f8fd931c40c5', 1, '2017-11-01 09:35:21', '2017-10-31 10:18:00', NULL, NULL, 4),
(8, '狗贼', '24318c1723b18b68f6d5961837d646c5', 1, '2017-11-01 09:35:21', '2017-10-31 10:18:00', NULL, NULL, 4),
(9, '涛妹', '0ca42d76135d73adb84cdaf80351e273', 1, '2017-11-01 09:35:00', '2017-10-30 16:00:00', NULL, NULL, 5),
(10, '小漠fdf', '9829567d9689807a1401435f609e76f4', 1, '2017-11-02 13:58:00', '2017-11-01 16:00:00', NULL, NULL, 4),
(12, '阿卡林3', '143ff35fa692532ecc9bdc090e7aa73c', 1, '2017-11-02 13:59:24', '2017-11-02 13:59:24', NULL, NULL, 3),
(13, '张三78', '8866bf87a79ae1ca677f5d747e8de866', 1, '2017-11-05 12:08:13', '2017-11-05 12:08:13', NULL, NULL, 3),
(14, '张三45', '09d0e4973dccd3e8c69d4b29af8a8976', 1, '2017-11-05 12:08:13', '2017-11-05 12:08:13', NULL, NULL, 3),
(15, 'a5', '8db42bfb3c7229cef48d30dcd17c96b7', 1, '2017-11-05 12:08:13', '2017-11-05 12:08:13', NULL, NULL, 3),
(16, '张三5gh', '55c027dc83849b7bfab3faf03da5dbe6', 1, '2017-11-05 12:08:13', '2017-11-05 12:08:13', NULL, NULL, 3),
(17, '张三df', 'f1b1e0579b5052b53a5e07cd202ede1a', 1, '2017-11-05 12:08:13', '2017-11-05 12:08:13', NULL, NULL, 3),
(18, '张三54', 'b5054f17f02714885ec9dd50486f6479', 1, '2017-11-05 12:08:13', '2017-11-05 12:08:13', NULL, NULL, 3),
(19, '张三132', '43f2174608d8c4dd3b1201c22ff25004', 1, '2017-11-05 12:08:13', '2017-11-05 12:08:13', NULL, NULL, 3),
(20, '张三321', 'f34b61041b4a37cc951e8aa90d4fac93', 1, '2017-11-05 12:08:13', '2017-11-05 12:08:13', NULL, NULL, 3);
...
ALTER TABLE `t_admin_login_log`
  ADD PRIMARY KEY (`id`),
  ADD KEY `t_admin_login_log_t_admin_id_fk` (`admin`);
...
ALTER TABLE `t_login_log`
  ADD PRIMARY KEY (`id`),
  ADD KEY `t_login_log_t_user_id_fk` (`user`);
...
--
ALTER TABLE `t_login_log`
  MODIFY `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键', AUTO_INCREMENT=52;
--
...
--
-- 限制表 `t_admin_login_log`
--
ALTER TABLE `t_admin_login_log`
  ADD CONSTRAINT `t_admin_login_log_t_admin_id_fk` FOREIGN KEY (`admin`) REFERENCES `t_admin` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `t_login_log`
  ADD CONSTRAINT `t_login_log_t_user_id_fk` FOREIGN KEY (`user`) REFERENCES `t_user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
