...
(
  id bigint auto_increment comment '主键'
    primary key,
  count int default '1' not null comment '楼层数',
  mobile_phone bigint not null comment '手机',
...
create procedure p_target_collection (IN `_mobile_phone` bigint, IN `_user` bigint)
  BEGIN
...
create procedure p_user_login (IN `_id` bigint, IN `_password` varchar(18))
  BEGIN
    DECLARE t_error INTEGER DEFAULT 0;
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET t_error=1;
